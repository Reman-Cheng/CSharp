...
attackname = "",armorname = "",speedname = "",handname = "",headname = "",wristname = "",shouldername="", cloalname="", legname = "",magicresistance = 0}
...
function status(self,p1)
    print("====你的❤️  "..p1.health.."/"..inithealth(self,article))
    print("====你的💙 "..p1.power.."/"..initpower(self))
    print("====你的📶 "..p1.level)
    print("====你的💰 "..p1.gold)
    print("====剩余📖 "..p1.experience)
    print("====你的🗡  "..p1.attack)
    print("====你的🛡  "..p1.armor)
    print("====你的👟 "..p1.speed)
    print("====你的💊 "..p1.lifegold)
end

addexpgloble = 0
p.addexp = function (self,p1,addexp)
    p1.experience = p1.experience + addexp
	while p1.experience >= p1.level*2 do
		p1.experience = p1.experience - p1.level*2
		p1.level = p1.level + 1
...
function prompt2(game)
	...
	for k,v in pairs(game[2])do
				if v[1] ~= "退出" then
					v[3] = v[3] or ""
					print(string.format("%2d",v[2])..": "..v[1].." 价格: "..string.format("%4d",v[3]).." 金币".." 加成: "..v[4].." "..v[5])
				else
					print(v[2]..": "..v[1])
			    end
			    if k % 4 == 0 then
					  ...
					  if ret ~= "退出" then
				        ...
				        elseif ret == "鸡你太美" then
				        	...
				        	if p1.power >=skills[2].mustkill[3] then
				                ...
				                print(p3.emojo.." "..p3.name.." 被☄️ 攻击并且直接生命值为: 0")
				                return p1,p3
				            else
				            	print("你的蓝量不足"..hurt.." 剩余："..p1.power.."/"..initpower(self))
	...
	for k,v in pairs(game[2])do
       if i == v[2] then
			    ...
			    if k ~= "quit" then
			    	if k ~= "mustkill" then
					v[3] = v[3] or ""
					print(string.format("%2d",v[2])..": "..v[1].." 消耗: "..v[3].." 蓝量".." 造成: "..v[3].." 点真实伤害")
				    else
					print(string.format("%2d",v[2])..": "..v[1].." 消耗: "..v[3].." 蓝量".." 功能: 直接秒杀怪物")
			        end
			    else
			    	print(string.format("%2d",v[2])..": "..v[1])
			    end
        ...
        if inputlife == "1" then  
		       ...
		       local i=0
				...
				do
		          ...
		          pload.attackname = tostring(pcopy[12])
		          pload.armorname = tostring(pcopy[13])
		          pload.speedname = tostring(pcopy[14])
		          pload.handname = tostring(pcopy[15])
		          pload.headname = tostring(pcopy[16])
		          pload.wristname = tostring(pcopy[17])
		          pload.shouldername = tostring(pcopy[18])
		          pload.cloalname = tostring(pcopy[19])
		          pload.legname = tostring(pcopy[20])
		        end
            -- handname = "",headname = "",wristname = "",shouldername="", cloalname="", legname = ""
